...
export function SelectionBoxWord({ children }: { children: React.ReactNode }) {
  return (
    <motion.span
      ...
      className="relative inline-block italic font-sans font-medium"
    >
      <span className="relative z-10 px-1.5 text-white">{children}</span>
      <span aria-hidden className="pointer-events-none absolute -inset-1.5 rounded-[6px] border border-white/70" />
